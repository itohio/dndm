...
var _ dialers.Remote = (*WireContext)(nil)
var _ dialers.Remote = (*Wire)(nil)

// WireContext is a wrapper over ReadWriter that will read and decode messages from Reader as well as encode and write them to the Writer. It allows
...
func NewWithContext(ctx context.Context, peer dialers.Peer, rw io.ReadWriter, handlers map[types.Type]dialers.MessageHandler) *WireContext {
	ret := &WireContext{
		Wire: New(peer, rw, handlers),
		read: contextRW{
...
func (c *WireContext) run(ctx context.Context) {
...
func (c *WireContext) Reader(ctx context.Context) io.Reader {
	return &reader{
		c:   &c.read,
		ctx: ctx,
	}
...
func (w *WireContext) Close() error {
	w.cancel()
...
func (w *WireContext) Read(ctx context.Context) (*types.Header, proto.Message, error) {
...
func (w *WireContext) Write(ctx context.Context, route routers.Route, msg proto.Message) error {
...
// Wire converts regular ReaderWriter into a Remote
type Wire struct {
	peer     dialers.Peer
...
func NewIO(peer dialers.Peer, r io.Reader, w io.Writer, handlers map[types.Type]dialers.MessageHandler) *Wire {
	rw := readWriter{r: r, w: w}
	return New(peer, rw, handlers)
}

// New creates a Remote using provided ReaderWriter.
func New(peer dialers.Peer, rw io.ReadWriter, handlers map[types.Type]dialers.MessageHandler) *Wire {
	return &Wire{
		peer:     peer,
		rw:       rw,
		handlers: handlers,
...
func (w *Wire) AddRoute(routes ...routers.Route) {
	w.mu.Lock()
...
func (w *Wire) DelRoute(routes ...routers.Route) {
	w.mu.Lock()
	for _, r := range routes {
...
func (w *Wire) Read(ctx context.Context) (*types.Header, proto.Message, error) {
...
func (w *Wire) Write(ctx context.Context, route routers.Route, msg proto.Message) error {
	buf, err := codec.EncodeMessage(msg, route)
	if err != nil {
		return err
	}
