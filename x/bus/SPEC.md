...
The `bus` package provides high-level, type-safe wrappers around the core DNDM API to reduce boilerplate code when implementing processing modules. These wrappers eliminate common patterns like type variable creation, type assertions, interest waiting, and context management.

**Package**: `github.com/itohio/dndm/x/bus`  
**Import**: `import "github.com/itohio/dndm/x/bus"`
...
```go
import "github.com/itohio/dndm/x/bus"

// Before: ~15 lines of boilerplate
// After: ~5 lines
producer, err := bus.NewProducer[*SensorData](ctx, router, "sensors.temperature")
if err != nil {
    return err
}
...
// After: ~5 lines
consumer, err := bus.NewConsumer[*SensorData](ctx, router, "sensors.temperature")
if err != nil {
    return err
}
...
consumer, _ := bus.NewConsumer[*SensorData](ctx, router, "sensors.temperature")
defer consumer.Close()
...
caller, err := bus.NewCaller[*GetStatusRequest, *GetStatusResponse](
    ctx, router,
    "commands.get_status",
...
caller, err := bus.NewCaller[*SubscribeRequest, *SubscribeResponse](
    ctx, router,
    "commands.subscribe",
    "responses.subscribe",
)
...
service, err := bus.NewService[*GetStatusRequest, *GetStatusResponse](
    ctx, router,
    "commands.get_status",
    "responses.get_status",
)
...
service, err := bus.NewService[*SubscribeRequest, *SubscribeResponse](
    ctx, router,
    "commands.subscribe",
    "responses.subscribe",
)
...
// Define inputs
cameraInput, _ := bus.AddInput[*CameraImage](module, "cameras.front")
sensorInput, _ := bus.AddInput[*SensorData](module, "sensors.imu")

// Define outputs
featuresOutput, _ := bus.AddOutput[*ImageFeatures](module, "image.features")
depthOutput, _ := bus.AddOutput[*DepthMap](module, "image.depth")
...
producer, _ := bus.NewProducer[*SensorData](ctx, router, "sensors.temperature")
defer producer.Close()
...
// Caller
caller, _ := bus.NewCaller[*GetStatusRequest, *GetStatusResponse](
    ctx, router,
    "commands.get_status",
    "responses.get_status",
)
...
// Service
service, _ := bus.NewService[*GetStatusRequest, *GetStatusResponse](
    ctx, router,
    "commands.get_status",
    "responses.get_status",
)
...
caller, _ := bus.NewCaller[*SubscribeRequest, *SubscribeResponse](
    ctx, router,
    "commands.subscribe",
    "responses.subscribe",
)
...
service, _ := bus.NewService[*SubscribeRequest, *SubscribeResponse](
    ctx, router,
    "commands.subscribe",
    "responses.subscribe",
)
...
input, _ := bus.AddInput[*CameraImage](module, "cameras.front")
output, _ := bus.AddOutput[*ImageFeatures](module, "image.features")
